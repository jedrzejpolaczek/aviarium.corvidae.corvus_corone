...
| problem_instance_ids | list[string] | yes | Ordered list of Problem Instance IDs included in this study, with pinned versions |
| algorithm_instance_ids | list[string] | yes | Ordered list of Algorithm Instance IDs included in this study, with pinned versions |
...
| test_type | string | yes | The statistical test that will evaluate it. Must be one of the tests in `docs/04-scientific-practice/01-methodology/02-statistical-methodology.md` §3, or `none` for an exploratory Study |
...
**Validation rules:**
- `test_type` must match a test named in `statistical-methodology.md` §3, or be exactly `none`
- `metric_id` must match a metric identifier in the metric taxonomy
- `test_type = "none"` declares an exploratory Study. The declaration is carried into the Report
  scope statement so that exploratory results are never presented as confirmatory

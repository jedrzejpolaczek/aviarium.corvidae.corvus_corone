...
| platform_snapshot | object | no | Captured automatically at Run start. Fields: `hostname: string`, `cpu_count: int`, `memory_gb: float`. Provides run-level hardware context for diagnosing performance variance across machines. `null` if capture failed. |

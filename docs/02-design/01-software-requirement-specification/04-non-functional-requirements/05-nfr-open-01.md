...
- **Operationalized in:** `docs/02-design/02-architecture/01-adr/adr-022-agpl-licence-and-dependency-policy.md` (code licence) and `docs/02-design/01-software-requirement-specification/05-constraints/04-const-technical.md` (dependency licences). The **data** licence for benchmark problems and experimental results is undecided; `docs/05-community/02-versioning-governance.md` §5 is where it will be recorded and is deferred (REF-TASK-0048)
...
- **Enforced by:** FR-22 (Raw Data export in open format), CONST-COM-01, CONST-COM-02, CONST-COM-03 (→ `docs/02-design/01-software-requirement-specification/05-constraints/01-index.md`)

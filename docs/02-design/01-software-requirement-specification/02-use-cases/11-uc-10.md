...
sequenceDiagram
    ...
    Learner->>Genealogy: Request history (e.g., "Show me the history of TPE")
    ...
    alt No genealogy data for algorithm
        ...
        Genealogy-->>Learner: Timeline: predecessor → limitation it addressed → next step in lineage
        Genealogy-->>Learner: Design choices distinguishing algorithm from predecessors

        Learner->>Genealogy: Navigate to ancestor or descendant algorithm (optional)
        Genealogy-->>Learner: History record for selected algorithm

        Note over Learner,Genealogy: Links to UC-07 (visualisation) and UC-08 (contextual help) for each algorithm in lineage
    end
```

...
| [CV-018](#cv-018) | Study entity references resolve and are not deprecated | Study, ProblemInstance, AlgorithmInstance | Status → locked | Reject |
| [CV-019](#cv-019) | ResultAggregate.metrics keys are defined metric identifiers | ResultAggregate | Write | Reject |
| [CV-020](#cv-020) | Hypothesis test_type and metric_id resolve | Study | Status → locked | Reject |
...
**Every entity a Study references must resolve, and must not be deprecated, at lock time**

| | |
|---|---|
| **Entities** | Study, ProblemInstance, AlgorithmInstance |
| **Check point** | Status transition — `lock_study()` |
| **Response** | Reject |

```
for id in study.problem_instance_ids + study.algorithm_instance_ids:
    assert exists(id) and not entity[id].deprecated
```

Every unresolved identifier is reported, not only the first (FR-27).

**Rationale:** entities are immutable and a revision is a new entity linked by `superseded_by`
(ADR-020), so an identifier that resolves at lock time resolves forever. That is what makes the
reference sufficient on its own and why no version needs to be recorded beside it. Checking at
lock rather than at write is deliberate: a draft Study is a working document.

**Error:** `ValidationError` — `"Study references {n} entities that do not resolve: {ids}"`

---

### CV-019

**Every key of ResultAggregate.metrics must be a defined metric identifier**

| | |
|---|---|
| **Entities** | ResultAggregate |
| **Check point** | Write time |
| **Response** | Reject |

```
for key in aggregate.metrics:
    assert key ∈ metric_taxonomy_identifiers
assert standard_reporting_set ⊆ aggregate.metrics.keys()
```

**Rationale:** the metric taxonomy is the only source of metric identifiers (ADR-012), and the
four Standard Reporting Set metrics are mandatory for every completed Experiment (ADR-008). An
aggregate carrying an unknown key cannot be compared with any other study.

**Error:** `UnknownMetricError` for an unknown key; `ValidationError` for a missing Standard
Reporting Set metric.

---

### CV-020

**Every pre-registered hypothesis must name a real test and a real metric**
...
```
for h in study.pre_registered_hypotheses:
    assert h.test_type ∈ statistical_methodology_tests
    assert h.metric_id ∈ metric_taxonomy_identifiers
```

**Rationale:** a hypothesis is a structured record precisely so that the Analyzer can compare
what was tested against what was declared and set `pre_registered` on the result (ADR-021).
Free-text hypotheses cannot support that flag, and the flag is already in the
`StatisticalTestResult` contract.

**Error:** `ValidationError` naming the offending hypothesis and field.

---

### CV-021

**The Problem Instance set must meet the diversity floor unless the Study is exploratory**

| | |
|---|---|
| **Entities** | Study, ProblemInstance |
| **Check point** | Status transition — `lock_study()` |
| **Response** | Reject, unless `study.study_type == "exploratory"` |

```
if study.study_type != "exploratory":
    assert len(study.problem_instance_ids) >= 5                    # ADR-009 D-1
    assert dimensionality_ranges_covered(study) >= 2               # ADR-009 D-2
    assert has_stochastic(study) and has_deterministic(study)      # ADR-009 D-3
```

For an exploratory Study the shortfall is recorded and carried into the limitations section of
both Reports instead of blocking execution.

**Rationale:** the escape hatch is what makes rejection acceptable. A Study scoped to one problem
class is legitimate science but is not confirmatory, and `study_type` is how the researcher says
which of the two they are running. Rejecting without the hatch would be an obstacle; warning
without it would let a confirmatory claim rest on five instances in a single dimensionality
range.

**Error:** `ValidationError` naming the failing rule, the shortfall, and both remedies (FR-30).

---

### CV-022

**An Experiment must carry a complete execution environment before any Run exists**

| | |
|---|---|
| **Entities** | Experiment, Run |
| **Check point** | Write time — when the Experiment is created |
| **Response** | Reject |

```
assert experiment.execution_environment.platform
assert experiment.execution_environment.hardware
assert experiment.execution_environment.language_version
```

**Rationale:** FR-10 requires the environment to be recorded, and the reproduction procedure in
the acceptance test strategy compares it. Recorded after the fact it describes whatever machine
happened to write the record, which is not the same claim.

**Error:** `ValidationError` naming the missing field.

---

### CV-023

**best_so_far must be monotone within a Run, in the objective direction**
...
```
direction = ProblemInstance[run.problem_instance_id].objective.type
for consecutive (a, b) in records_of(run):
    assert b.best_so_far <= a.best_so_far  if direction == "minimize"
    assert b.best_so_far >= a.best_so_far  if direction == "maximize"
```

**Rationale:** `best_so_far` is a running extremum by construction (ADR-023), and every anytime
metric reconstructs curves from it under LOCF (ADR-003). LOCF is exact only because the quantity
is constant between improvements; a non-monotone sequence silently invalidates every anytime
